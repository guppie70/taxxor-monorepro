...
public class DebugMiddleware : ProjectLogic
{
    ...
    public async Task Invoke(HttpContext context)
    {
        ...
        // if (isDevelopmentEnvironment || reqVars.isDebugMode) appLogger.LogInformation("** In Debug Middleware **");

        if (!reqVars.isStaticAsset)
        {

            if (!IsInternalServicePage(reqVars) && (isDevelopmentEnvironment || reqVars.isDebugMode) && (reqVars.pageId != "filingeditorlistlocks") || reqVars.pageId == "cms-overview")
            {
                ...
                if (!projectVars.isInternalServicePage)
                {
                    ...
                    //
                    if (((reqVars.pageId.Contains("dynamic_resourcesxyz") || reqVars.pageId == "xyz") && reqVars.isDebugMode) || reqVars.pageId == "xyz")
                    {
                ...
                if (debugStoreInspectorLogFiles && reqVars.thisUrlPath.EndsWith(".html"))
                {
                    ...
                    try
                    {
                        ...
                        // Dump the files
                        await TextFileCreateAsync(PrettyPrintXml(xmlApplicationConfiguration.OuterXml), inspectorLogFolderPathOs + "/application-configuration.xml");
                        await TextFileCreateAsync(PrettyPrintXml(reqVars.xmlHierarchy.OuterXml), inspectorLogFolderPathOs + "/site-structure.xml");

                        if (projectVars.currentUser.XmlUserPreferences.ChildNodes.Count > 0) await TextFileCreateAsync(PrettyPrintXml(projectVars.currentUser.XmlUserPreferences.OuterXml), inspectorLogFolderPathOs + "/user-preferences.xml");
                    }
                    catch (Exception ex)
                    {
            ...
            {
                //
                // Dump the incoming request for inspection coming from internal requests
                //
                if (reqVars.pageId == "serviceinformationnladiebla")
                {
                    appLogger.LogInformation("-------- Incoming Request Information --------");
                    Console.WriteLine(await DebugIncomingRequest(context.Request));
                    Console.WriteLine("");
        ...
        else
        {
            ...
            {
                // appLogger.LogInformation($"*------- Static Asset ({reqVars.rawUrl}) -------*");
            }
        }
