...
<xsl:stylesheet exclude-result-prefixes="xs aps" version="3.0" xmlns:xs="http://www.w3.org/2001/XMLSchema"
	...
	<xsl:template match="div[@data-charttype]//td/text()" priority="2" mode="lorem">
		<xsl:choose>
			<xsl:when test="ancestor::td[1]/@format = 'rate-indicator'">
				<xsl:value-of select="aps:getRandomInt(if(aps:getRandomBoolean()) then(-5) else(5))"/>
				<xsl:text>%</xsl:text>
	...
	<xsl:function name="aps:getRandomBoolean" as="xs:boolean">
		<xsl:variable name="random" as="map(*)" select="random-number-generator()"/>
		<xsl:value-of select="$random?number >= 0.5"/>
	</xsl:function>

	<xsl:function name="aps:getRandomInt" as="xs:double">
		<xsl:param name="max" as="xs:double"/>
		<xsl:variable name="random" as="map(*)" select="random-number-generator()"/>
		<xsl:value-of select="round(($max) * $random?number)"/>
	</xsl:function>
	...
	<xsl:function name="aps:getRandomDouble" as="xs:double">
		...
		<xsl:variable name="random" as="map(*)" select="random-number-generator()"/>
		<xsl:value-of select="round-half-to-even(($max) * $random?number, $decimals)"/>
	</xsl:function>
	...
	<xsl:template name="aps:getRandomText">
		...
		<xsl:variable name="p-random" select="round($p-count * $random?number)"/>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:variable name="randomtwo" as="map(*)" select="random-number-generator()"/>
				<xsl:variable name="text-start" select="$text-length-p - $text-length"/>
				<xsl:variable name="text-random-start" select="ceiling($randomtwo?number * $text-start)"/>
