...
namespace FrameworkLibrary
{
    ...
    /// </summary>
    public static class FrameworkEncryption
    {
        /// <summary>
        ...
        {
            string input = string.IsNullOrEmpty(value) ? "" : value;
        ...
        {
            string input = string.IsNullOrEmpty(value) ? "" : value;
            byte[] byInput = Encoding.UTF8.GetBytes(input);
            byte[] byKey = Encoding.UTF8.GetBytes(key);
            HMACSHA1 objHmac = new HMACSHA1(byKey);
            //hmac.Key = bskey;
