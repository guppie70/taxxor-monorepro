...
public abstract partial class Framework
{
    ...
    public static string GetInnerText(string input, string start, string end)
    {
        ...
        {
            String[] arrTemp1 = input.Split(new String[] { start }, StringSplitOptions.RemoveEmptyEntries);
            if (arrTemp1.Length > 0)
            {
                String temp = arrTemp1[1];
                if (temp.Contains(end))
                {
                    String[] arrTemp2 = temp.Split(new String[] { end }, StringSplitOptions.RemoveEmptyEntries);
                    if (arrTemp1.Length > 0)
                    {
                        result = arrTemp2[0];
                    }
    ...
    {
        if (string.IsNullOrEmpty(value) || value.Length < l) return value;
    ...
    public static string ReplaceAttributeContent(string htmlSnippet, string attributeName, string newValue, bool logWarnings = false)
    {
        ...
        if (startIndex == -1)
        {
            if (logWarnings) Console.WriteLine($"Attribute with name {attributeName} not found in HTML snippet");
            return htmlSnippet;
        }
        ...
        if (endIndex == -1)
        {
            if (logWarnings) Console.WriteLine($"Closing quote not found for attribute value in attribute name {attributeName}");
            return htmlSnippet;
        }
...
public static class StringExtensions
{
    ...
    {
        return ContainsAny(input, StringComparison.CurrentCulture, containsKeywords);
    }
    ...
    {
        return EndsWithAny(input, StringComparison.CurrentCulture, containsKeywords);
    }
    ...
    public static bool IsBase64(this string base64String, bool logError = false)
    {
        ...
        else
        {
            ...
            catch (Exception exception)
            {
                ...
                if (logError) Console.WriteLine($"Could not test if string was base 64 encoded... error: {exception.ToString()}");
            }
            return false;
        }
    }
