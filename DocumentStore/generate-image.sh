...
application_name="documentstore"
git_commit=""
...
cp ../../_utils/Certificates/Taxxor_root.crt ./DocumentStore
cp ../../_utils/Certificates/documentstore.pfx ./DocumentStore
...
sed -iE "s/\(<repository id=.application-root. version=.\).*\(. \/><repository id=.frame\)/\1$application_version\2/" "$( pwd )/DocumentStore/_repro-info.xml"
rm "$( pwd )/DocumentStore/_repro-info.xmlE"
...
# Use docker-container builder for cache export support
